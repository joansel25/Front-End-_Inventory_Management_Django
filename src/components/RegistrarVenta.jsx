...
export default function RegistrarVenta() {
          ...
          try {
            ...
            const empleadoEncontrado = empleadosRes.data.find(emp => {
              console.log("🔎 Comparando:", {
                empleadoUsuarioId: emp.usuario?.id,
                empleadoId: emp.id,
                userIdToken: payload.user_id,
                empleadoNombre: emp.nombre,
                usuarioNombre: emp.usuario?.username
              });
              
              // Verificar si el usuario del empleado coincide con el user_id del token
              return emp.usuario?.id === payload.user_id;
            });
            ...
            if (empleadoEncontrado) {
              ...
              console.log("✅ Empleado encontrado:", {
                ...
                usuario_id: empleadoEncontrado.usuario?.id
              });
            } else {
              console.warn("❌ No se encontró empleado para el usuario:", payload.user_id);
              console.log("📊 Empleados disponibles:", empleadosRes.data.map(e => ({
                id: e.id,
                nombre: e.nombre,
                usuario_id: e.usuario?.id,
                usuario_nombre: e.usuario?.username
              })));
            }
          } catch (empleadoError) {
  ...
  const validarVenta = () => {
    ...
    if (!empleadoId) {
      alert("❌ No se pudo identificar al empleado. Contacta al administrador.");
      return false;
    }
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        if (errorData.id_empleado) {
          mensajeError = `❌ Error en empleado: ${Array.isArray(errorData.id_empleado) 
            ? errorData.id_empleado.join(", ") 
            : errorData.id_empleado}`;
          
          mensajeError += `\n\n🔧 Información de depuración:`;
          mensajeError += `\n• Empleado ID enviado: ${empleadoId}`;
          mensajeError += `\n• User ID del token: ${JSON.parse(atob(localStorage.getItem("access").split(".")[1])).user_id}`;
          mensajeError += `\n• Usuario: ${userName}`;
      ...
      <div className="container py-4">
        ...
        {!empleadoId && (
          <div className="alert alert-warning text-center">
            <strong>⚠️ Advertencia:</strong> No se pudo identificar tu usuario como empleado. 
                        ...
                        <div className="col-md-6 text-end">
                          ...
                          <div className="d-flex gap-2 justify-content-end">
                            <button
                              ...
                              disabled={loading || venta.detalles.length === 0 || !empleadoId}
                            >
                              {loading ? (
                                <>
                                  <div
